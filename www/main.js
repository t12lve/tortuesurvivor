...
loadingManager.onProgress = function(url, itemsLoaded, itemsTotal) {
    textureLoaded = itemsLoaded;
    textureTotal = itemsTotal;
    updateLoadingScreen();
};
loadingManager.onLoad = function() {
    texturesDone = true;
    if (textureTotal > 0) textureLoaded = textureTotal;
    updateLoadingScreen();
};
...
function playTitleMusicNow() {
    ...
    if (!ostAudio.src || !ostAudio.src.includes('Title Theme')) {
    ...
    const tryPlay = () => {
        ostAudio.play().then(() => {
            ...
            updateNowPlayingTitle('Title Theme Tortue Survivor');
        }).catch(e => console.log('OST boot play:', e));
    };
    if (ostAudio.readyState >= 2) tryPlay();
    else {
        ostAudio.addEventListener('canplaythrough', tryPlay, { once: true });
        ostAudio.load();
        tryPlay();
    }
}
...
function finishBootLoading() {
    ...
    if (gameState === 'START') playTitleMusicNow();
}

function updateLoadingScreen() {
    const progressBar = document.getElementById('loading-progress-bar');
    const total = textureTotal + audioTotal;
    const loaded = Math.min(textureLoaded, textureTotal || textureLoaded) + audioLoaded;
    if (progressBar && total > 0) {
        progressBar.style.width = Math.min(100, (loaded / total) * 100) + '%';
    }
    const texturesOk = texturesDone || (textureTotal > 0 && textureLoaded >= textureTotal);
    const audioOk = audioLoaded >= audioTotal;
    if (texturesOk && audioOk && total > 0) {
        finishBootLoading();
    }
}
...
function generatePropsForChunk(chunk, isInit = false) {
    ...
    for(let i = 0; i < numProps; i++) {
        ...
        let map, width=3.2, height=3.2, isWall = false;
...
function init() {
    ...
    // 3. Load Assets
    sprites.leftback = loadPixelTexture('source/leftback.png');
    sprites.leftfront = loadPixelTexture('source/leftfront.png');
    sprites.rightback = loadPixelTexture('source/rightback2.png');
    sprites.rightfront = loadPixelTexture('source/rightfront.png');
    
    sprites.journaR = loadPixelTexture('source/journaR.png');
    sprites.journaRm = loadPixelTexture('source/journaRm.png');

    sprites.usR = loadPixelTexture('source/usright.png');
    ...
    sprites.poliRI = loadPixelTexture('source/poliRI.png');
    sprites.glucksR = loadPixelTexture('source/glucksR.png');
    
    sprites.billet_vert = loadPixelTexture('source/billet_vert.png');
    sprites.isfclim = loadPixelTexture('source/isfclim.png');
    ...
    sprites.marineR = loadPixelTexture('source/marinelpdroite.png');
    sprites.tondR = loadPixelTexture('source/tondR.png');

    sprites.mail = loadPixelTexture('source/mail.png');
    sprites.laitfraise = loadPixelTexture('source/laitfraise.png');
    sprites.maillotinvul = loadPixelTexture('source/maillotinvul.png');
    sprites.erepu6 = loadPixelTexture('source/6erepu.png');
    sprites.aimant = loadPixelTexture('source/aimant.png');
    sprites.colis = loadPixelTexture('source/colis.png');
    sprites.roussoR = loadPixelTexture('source/roussoR.png');
    sprites.rimaR = loadPixelTexture('source/rimaR.png');
    sprites.poutouR = loadPixelTexture('source/poutouR.png');
    sprites.ruffin = loadPixelTexture('source/ruffin.png');
    sprites.renaut = loadPixelTexture('source/renaut.png');
    sprites.fn = loadPixelTexture('source/fn.png');
    sprites.boots = loadPixelTexture('source/boots.png');

    // Make textures crisp (pixel art)
    Object.values(sprites).forEach(tex => makePixelTexture(tex));

    // 4. Create Player
    const playerGeo = new THREE.PlaneGeometry(3.4, 3.4); // Portrait readability
    ...
    window.addEventListener('pointerdown', (e) => {
        ...
        // Touch uses virtual joystick only — pointer click-to-move fights diagonals
    ...
    window.addEventListener('pointermove', (e) => {
        if (pointerActive && e.pointerType !== 'touch') updatePointerTarget(e);
    });
...
function updatePlayer(delta) {
    ...
    // Mobile Virtual Joystick (exclusive — do not mix with pointer click-to-move)
    if (typeof joystickActive !== 'undefined' && joystickActive) {
        dx += joystickDx;
        dy += joystickDy;
    } else if (pointerActive) {
        // Desktop mouse click-to-move only
    ...
    // Normalize any 2D input (keeps diagonals at full speed)
    if (dx !== 0 || dy !== 0) {
        const length = Math.sqrt(dx * dx + dy * dy);
        if (length > 0) {
            dx /= length;
...
function spawnEnemy(forcedType = null) {
    ...
    let geo = new THREE.PlaneGeometry(2.5, 2.5); // Portrait readability
...
function dropLoot(pos) {
    ...
    let type = 'xp';
    let size = 0.42; // slight upscale after camera zoom
    let tex;
...
function spawnSpecialItem(type, iconName) {
    ...
    const bgGeo = new THREE.PlaneGeometry(3.4, 3.4);
    ...
    const iconWidth = 2.1 * Math.min(1, aspectRatio);
    const iconHeight = 2.1 * Math.min(1, 1/aspectRatio);
...
function generatePowerChoices(container) {
    ...
    choices.forEach((power, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="power-icon" style="${isGolden ? 'filter: drop-shadow(0 0 10px #f1c40f);' : ''}">${power.icon}</div>
            <div class="power-card-body">
                <h3 style="${isGolden ? 'color: #f1c40f; text-shadow: 0 0 5px #f1c40f;' : ''}">${power.name} (Lvl ${nextLevel})</h3>
                <div class="power-cat" style="color: ${power.category==='Offensif'?'#e74c3c':(power.category==='Défensif'?'#3498db':'#2ecc71')};">${power.category}</div>
                <p class="desc">${power.desc}</p>
                <p class="power-level-gain" style="color: ${isGolden ? '#f1c40f' : '#2ecc71'};">${levelGainText}</p>
            </div>
        `;
...
// Particle System — shared geometry to cut GC / stutter on Android
...
function getPortraitFrustum(aspect) {
    ...
    const halfW = landscapeHalfW * 0.38;
...
ostAudio.addEventListener('ended', () => {
    if (gameState === 'START') {
        // Loop title theme immediately (no artificial delay)
        playTitleMusicNow();
    } else {
        playRandomOst();
    }
...
window.addEventListener('touchmove', (e) => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.identifier === joystickTouchId) {
            ...
            if (dist > 5) {
                // Use raw axes (not only clamped knob) so all diagonals stay faithful
                const nx = rawDx / Math.max(dist, 0.0001);
                const ny = rawDy / Math.max(dist, 0.0001);
                const strength = Math.min(1, dist / maxJoystickRadius);
                joystickDx = nx * strength;
                joystickDy = -ny * strength; // screen Y down → world Y up
            } else {
                joystickDx = 0;
                joystickDy = 0;
            }
...
document.getElementById('quit-confirm-btn').addEventListener('click', () => {
    // Electron desktop quit
    if (typeof require !== 'undefined') {
        try {
            const { ipcRenderer } = require('electron');
            if (ipcRenderer) { ipcRenderer.send('window-close'); return; }
        } catch (e) {}
    }
    window.close();
    // Android fallback: exit app
    exitNativeApp();
});
...
document.getElementById('exit-app-confirm-btn')?.addEventListener('click', () => {
    document.getElementById('exit-app-modal')?.classList.add('hidden');
    exitNativeApp();
});
document.getElementById('exit-app-cancel-btn')?.addEventListener('click', () => {
    document.getElementById('exit-app-modal')?.classList.add('hidden');
    backExitPressCount = 0;
});
...
document.addEventListener('mousemove', (e) => {
    if (gameState === 'START' || gameState === 'LEVELUP') {
        ...
        if (gameState === 'START') {
            ...
            if (stage) stage.style.transform = `translate(${x * 0.35}px, ${y * 0.35}px)`;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof lucide !== 'undefined') lucide.createIcons();
    tryRestoreSession();
    initMinecraftLogoEffects();
});

const MC_SPLASH_LINES = [
    'Aussi essayez TortueCraft!',
    '100% République!',
    'SMIC à 1700€!!',
    'Now with more turtle!',
    'Vive la fraaaaaance!',
    'Pixel Perfect!',
    'Glucksman en colère!',
    'Dash or die!',
    'Made in Melon!',
    '6ème République ASAP!',
...
function fitMenuLogoToStage() {
    const stage = document.getElementById('logo-stage');
    const logo = document.getElementById('main-logo');
    if (!stage || !logo) return;
    const rect = stage.getBoundingClientRect();
    if (rect.height < 40 || rect.width < 40) return;
    // Explicit px so the logo truly fills status-bar → Jouer
    logo.style.maxHeight = Math.floor(rect.height) + 'px';
    logo.style.maxWidth = Math.floor(rect.width) + 'px';
    logo.style.height = 'auto';
    logo.style.width = 'auto';
}

function spawnLogoSpark() {
    const layer = document.getElementById('logo-spark-layer');
    if (!layer) return;
    ...
    spark.style.width = spark.style.height = (4 + Math.floor(Math.random() * 5)) + 'px';
    layer.appendChild(spark);
    setTimeout(() => spark.remove(), 800);
}

function refreshMcSplash() {
    const el = document.getElementById('mc-splash');
    if (!el) return;
    const line = MC_SPLASH_LINES[Math.floor(Math.random() * MC_SPLASH_LINES.length)];
    el.textContent = line;
    el.style.right = (2 + Math.random() * 10) + '%';
    el.style.bottom = (4 + Math.random() * 14) + '%';
    el.style.transform = `rotate(${-24 + Math.random() * 14}deg)`;
}

function applyRandomLogoJolt() {
    const logo = document.getElementById('main-logo');
    if (!logo || gameState !== 'START') return;
    const rot = (Math.random() * 8 - 4).toFixed(1);
    const scale = (1.02 + Math.random() * 0.08).toFixed(3);
    logo.style.animation = 'none';
    logo.style.filter = `drop-shadow(0 6px 18px rgba(0,0,0,0.85)) hue-rotate(${Math.floor(Math.random() * 40)}deg)`;
    logo.style.transform = `rotate(${rot}deg) scale(${scale})`;
    setTimeout(() => {
        if (!logo) return;
        logo.style.filter = 'drop-shadow(0 6px 18px rgba(0,0,0,0.85))';
        logo.style.transform = '';
        logo.classList.add('mc-alive');
        logo.style.animation = '';
    }, 180);
}

function initMinecraftLogoEffects() {
    const logo = document.getElementById('main-logo');
    if (!logo) return;
    logo.classList.add('mc-alive');
    const scheduleFit = () => requestAnimationFrame(() => {
        fitMenuLogoToStage();
        requestAnimationFrame(fitMenuLogoToStage);
    });
    if (logo.complete) scheduleFit();
    else logo.addEventListener('load', scheduleFit, { once: true });
    scheduleFit();
    refreshMcSplash();
    ...
    setInterval(() => {
        ...
        for (let i = 0; i < 3 + Math.floor(Math.random() * 4); i++) {
            setTimeout(spawnLogoSpark, i * 60);
        }
    }, 2800 + Math.floor(Math.random() * 1200));

    setInterval(() => {
        if (gameState !== 'START') return;
        if (Math.random() < 0.55) spawnLogoSpark();
    }, 450);
}
...
function snapshotSession() {
    ...
    try {
        const data = {
            ...
            activePowers: activePowers.map(p => ({ id: p.id, level: p.level, name: p.name })),
...
let ostWasPlayingBeforeHide = false;
let gamePausedByBackground = false;
let backExitPressCount = 0;
...
function muteAppAudio() {
    ostWasPlayingBeforeHide = !!(ostAudio && !ostAudio.paused);
    if (ostAudio && !ostAudio.paused) {
        try { ostAudio.pause(); } catch (e) {}
    }
    ...
    const winVid = document.getElementById('victory-video');
    if (winVid && !winVid.paused) {
        try { winVid.pause(); } catch (e) {}
    }
}

function restoreAppAudio() {
    if (audioSuspendedByBackground && audioCtx) {
        audioCtx.resume().catch(() => {});
        audioSuspendedByBackground = false;
    }
    const bg = document.getElementById('bg-video');
    if (bg && gameState === 'START' && document.getElementById('start-screen') && !document.getElementById('start-screen').classList.contains('hidden')) {
        bg.play().catch(() => {});
    }
    if (ostWasPlayingBeforeHide && ostAudio && (gameState === 'START' || gameState === 'PLAYING')) {
        ostAudio.play().catch(() => {});
        setMusicToggleIcons(false);
    }
    ostWasPlayingBeforeHide = false;
}

function onAppBackground() {
    // Ignore transient hide events before the menu has finished booting
    if (!bootFinished) return;
    muteAppAudio();
    if (gameState === 'PLAYING') {
        gamePausedByBackground = true;
        gameState = 'PAUSED';
        document.getElementById('pause-screen')?.classList.remove('hidden');
        const banner = document.getElementById('breaking-news-banner');
        if (banner) banner.classList.add('paused');
...
function onAppForeground() {
    restoreAppAudio();
    // Leave pause screen up if we auto-paused — player chooses Resume
    gamePausedByBackground = false;
}
...
function showBackExitToast() {
    ...
    backToastTimer = setTimeout(() => toast.classList.add('hidden'), 2000);
}

function openExitAppPrompt() {
    document.getElementById('exit-app-modal')?.classList.remove('hidden');
    backExitPressCount = 0;
    const toast = document.getElementById('back-exit-toast');
    if (toast) toast.classList.add('hidden');
}

function closeTopOverlayIfAny() {
    const exitModal = document.getElementById('exit-app-modal');
    if (exitModal && !exitModal.classList.contains('hidden')) {
        exitModal.classList.add('hidden');
        return true;
    }
    const quitModal = document.getElementById('quit-prompt-modal');
    ...
    const subMenus = ['help-menu', 'options-menu', 'enemies-menu', 'about-menu', 'bonus-menu', 'attacks-menu'];
...
function requestExitViaBack() {
    backExitPressCount++;
    clearTimeout(backExitTimer);
    backExitTimer = setTimeout(() => { backExitPressCount = 0; }, 2200);

    if (backExitPressCount >= 2) {
        openExitAppPrompt();
        return;
    }
    showBackExitToast();
...
function handleAndroidBackButton() {
    if (closeTopOverlayIfAny()) return;
    ...
    if (gameState === 'LEVELUP') {
        // Don't skip level-up with back — treat as exit intent
        requestExitViaBack();
        return;
    }

    // START / PAUSED / GAMEOVER / VICTORY → multi-back + confirm
    requestExitViaBack();
}
...
async function exitNativeApp() {
    ...
    if (App && typeof App.exitApp === 'function') {
        try {
            ...
            console.log('exitApp', e);
        }
    }
    window.close();
}

let androidLifecycleReady = false;

async function setupAndroidAppLifecycle() {
    if (androidLifecycleReady) return;
    const App = getCapacitorApp();
    if (!App) {
        console.log('Capacitor App plugin unavailable — visibility fallback only');
        return;
    }
    androidLifecycleReady = true;
    try {
        await App.addListener('appStateChange', ({ isActive }) => {
            if (!isActive) onAppBackground();
            else onAppForeground();
        });
    } catch (e) { console.log('appStateChange', e); }

    try {
        await App.addListener('pause', () => onAppBackground());
        await App.addListener('resume', () => onAppForeground());
    } catch (e) { /* optional on some platforms */ }
...
window.addEventListener('DOMContentLoaded', () => {
    setupAndroidAppLifecycle();
});
// Cap bridge may load slightly after first DOMContentLoaded
setTimeout(setupAndroidAppLifecycle, 400);
setTimeout(setupAndroidAppLifecycle, 1500);
